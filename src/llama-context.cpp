...
llama_context::llama_context(
    ...
    if (!hparams.vocab_only) {
        llama_memory_params params_mem = {
            ...
            /*.kv_tier_semantic_query_prefix=*/ params.kv_tier_semantic_query_prefix,
            /*.kv_tier_semantic_doc_prefix =*/ params.kv_tier_semantic_doc_prefix,
            /*.kv_tier_semantic_threshold  =*/ params.kv_tier_semantic_threshold,
            /*.kv_tier_semantic_topk       =*/ params.kv_tier_semantic_topk,
            /*.kv_tier_semantic_parallel   =*/ params.kv_tier_semantic_parallel,
            /*.kv_tier_semantic_threads    =*/ params.kv_tier_semantic_threads,
            /*.kv_tier_paged_blocks        =*/ params.kv_tier_paged_blocks,
            /*.kv_tier_paged_block_size    =*/ params.kv_tier_paged_block_size,
            /*.kv_tier_cold_resume         =*/ params.kv_tier_cold_resume,
            /*.kv_tier_instance_id         =*/ params.kv_tier_instance_id,
            /*.kv_tier_cold_budget_mb      =*/ params.kv_tier_cold_budget_mb,
...
void llama_context::resolve_fused_ops(const llama_memory_context_i * mctx, uint32_t n_seqs) {
    ...
    auto resolve = [&](const llm_fused_op_probe & probe, bool & enabled) {
        ...
        if (device_mismatch) {
            ...
            //
            // NOTE(fork): the 2026-07-31 upstream sync folded the per-op auto
            // resolution into this generic lambda, so the guard is scoped to
            // the flash-attention probe here to keep its original semantics.
            if (probe.op == LLM_FUSED_OP_FLASH_ATTN && model.wp_pager != nullptr && model.n_devices() > 1) {
                throw std::runtime_error(
...
void llama_context::set_embeddings_layer_inp(uint32_t lid, bool enable) {
    ...
    // bound against the container, not n_layer(), so this stays correct if the
    // +1 head-input slot sizing ever changes. Equivalent to upstream's
    // "lid <= n_layer()" for the current sizing.
...
ggml_status llama_context::graph_compute(
            ggml_cgraph * gf,
                   bool   batched) {
    // Callers that do not go through process_ubatch (staged path, reserve) leave
    // sched_active null; they always mean this context's own scheduler.
    if (sched_active == nullptr) {
        sched_active = sched.get();
    }
        ...
        if (ubatch.n_tokens < 32 || full_offload) {
            if (wp_hetero && backend_paging != nullptr) {
                ...
                const bool ffn_island =
                    ...
                    strcmp(name, "l_out") == 0 ||
                    // upstream renamed the DS4 per-layer output tensor l_out -> l_last
                    // in the 2026-07-31 sync; match both or the island pin silently
                    // stops firing on DeepSeek V4 and the MoE chain restages over TB3.
                    strcmp(name, "l_last") == 0 ||
...
llama_context_params llama_context_default_params() {
    llama_context_params result = {
        ...
        /*.kv_tier_semantic_query_prefix=*/ nullptr,
        /*.kv_tier_semantic_doc_prefix =*/ nullptr,
        /*.kv_tier_semantic_threshold  =*/ 0.65f,
        /*.kv_tier_semantic_topk       =*/ 5,
        /*.kv_tier_semantic_parallel   =*/ 1,
        /*.kv_tier_semantic_threads    =*/ 0,
        /*.kv_tier_paged_blocks        =*/ false,
        /*.kv_tier_paged_block_size    =*/ 0,
        /*.kv_tier_cold_resume         =*/ false,
...
llama_context * llama_init_from_model(
    ...
    // NOTE(fork): the 2026-07-31 upstream sync added a generic
    // "quantized V cache requires flash_attn" resolver above (AUTO -> ENABLED,
    // explicit DISABLED -> error), which subsumes the turbo *V* case and our old
    // duplicate error check. It keys on type_v only, so a turbo *K* cache paired
    // with an unquantized V still needs this. Semantics follow upstream now:
    // an explicit --flash-attn off is an error rather than a silent override.
