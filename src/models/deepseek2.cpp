...
llama_model_deepseek2::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    for (int32_t il = il_first; il <= il_last; ++il) {
        ...
        // il_last, not n_layer - 1: under a pipeline band this stage's last layer
        // is not the model's last layer.
        if (il == il_last && inp_out_ids && (!cparams.embeddings_nextn || cparams.embeddings_nextn_masked)) {
            cur   = ggml_get_rows(ctx0, cur, inp_out_ids);
