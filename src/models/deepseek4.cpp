...
void llama_model_deepseek4::load_arch_hparams(llama_model_loader & ml) {
    // NextN/MTP: DS4 ships the MTP block as extra blocks beyond the main stack,
    // so block_count = n_layer + n_nextn. Read this FIRST: n_layer() is
    ...
    // In-GGUF DSpark head: fc.weight is the main_proj that fuses the target-layer
    // taps. Its presence is what distinguishes a DSpark-bearing DS4 checkpoint.
...
void llama_model_deepseek4::load_arch_tensors(llama_model_loader & ml) {
    ...
    // Runs to n_layer_all so the nextn blocks (i >= n_layer) are LOADED. They are
    // not executed by the trunk: the graph's main loop runs i < n_layer. On
    // DS4-Flash-0731 those are the three DSpark stages, which carry full DS4
    // bodies (compress_ratios[43..45] == 0, so no compressor/indexer tensors).
    for (int i = 0; i < n_layer_all; ++i) {
        ...
        // on the workers, so the spine must not load them. OR-ed with upstream's
        // per-layer flags so the nextn/trunk SKIP logic still applies.
        const int expert_flags = routed_experts_external ? TENSOR_SKIP | TENSOR_NOT_REQUIRED : 0;
        layer.ffn_gate_exps = create_tensor(tn(LLM_TENSOR_FFN_GATE_EXPS, "weight", i), {n_embd,   n_ff_exp, n_expert}, expert_flags | flags);
        layer.ffn_down_exps = create_tensor(tn(LLM_TENSOR_FFN_DOWN_EXPS, "weight", i), {n_ff_exp, n_embd,   n_expert}, expert_flags | flags);
        layer.ffn_up_exps   = create_tensor(tn(LLM_TENSOR_FFN_UP_EXPS,   "weight", i), {n_embd,   n_ff_exp, n_expert}, expert_flags | flags);

        layer.ffn_gate_shexp = create_tensor(tn(LLM_TENSOR_FFN_GATE_SHEXP, "weight", i), {n_embd,                     n_ff_exp * n_expert_shared}, flags);
        layer.ffn_down_shexp = create_tensor(tn(LLM_TENSOR_FFN_DOWN_SHEXP, "weight", i), {n_ff_exp * n_expert_shared, n_embd                    }, flags);
        layer.ffn_up_shexp   = create_tensor(tn(LLM_TENSOR_FFN_UP_SHEXP,   "weight", i), {n_embd,                     n_ff_exp * n_expert_shared}, flags);

        // NextN head. Upstream marks eh_proj/enorm/hnorm REQUIRED because it only
        // ever saw 1-block MTP. DS4-Flash-0731's three DSpark stages have NONE of
        // them -- eh_proj is MTP's input projection and DSpark has no analogue --
        // so requiring them fails the load outright on a DSpark checkpoint. They
        // are TENSOR_NOT_REQUIRED here; graph_mtp already GGML_ASSERTs on each one,
        // so a genuinely malformed MTP model still fails, just at graph build with
        // a better message instead of at load.
        if (i >= n_layer) {
            ...
            // OURS: the last DSpark stage carries the collapsed hyper-connection
            // head (blk.45.nextn.hc_head_{fn,base,scale} on DS4-Flash-0731).
            if (i == n_layer_all - 1) {
    ...
    // OURS: in-GGUF DSpark head (main_proj + markov + confidence).
    if (ml.get_tensor_meta("fc.weight") != nullptr) {
...
static void dsv4_build_dspark_head(llm_graph_context & g, const llama_model & model, ggml_tensor * tokens) {
    ...
    const int64_t n_tokens = res->t_logits->ne[1];
    const int64_t block_size = model.dflash_block_size;
    const int64_t n_blocks = g.ubatch.n_seqs_unq;
    if (n_blocks <= 0 || n_tokens % n_blocks != 0 || n_tokens / n_blocks > block_size) {
        return;
    }

    const int64_t block_tokens = n_tokens / n_blocks;
    const size_t token_stride = block_tokens * tokens->nb[0];
    const size_t logits_stride = block_tokens * res->t_logits->nb[1];
    ggml_tensor * prev = ggml_cont_1d(ctx0, ggml_view_2d(ctx0, tokens, 1, n_blocks, token_stride, 0), n_blocks);
    ...
    for (int64_t i = 0; i < block_tokens; ++i) {
        ...
        ggml_tensor * base = ggml_view_2d(ctx0, res->t_logits, n_vocab, n_blocks,
                logits_stride, i * res->t_logits->nb[1]);
        ggml_tensor * col = ggml_add(ctx0, base, bias);
        logits = logits ? ggml_concat(ctx0, logits, col, 1) : col;

        ggml_tensor * embd_i = ggml_view_2d(ctx0, res->t_embd, res->t_embd->ne[0], n_blocks,
                block_tokens * res->t_embd->nb[1], i * res->t_embd->nb[1]);
        ggml_tensor * feat = ggml_concat(ctx0, ggml_cont(ctx0, embd_i), markov_embd, 0);
    ...
    logits = ggml_reshape_3d(ctx0, logits, n_vocab, n_blocks, block_tokens);
    ...
    ggml_build_forward_expand(g.gf, confidence);
}
...
ggml_tensor * llama_model_deepseek4::graph::get_constant(
        ggml_type type,
    ...
    for (const constant_cache_entry & entry : constant_cache) {
        bool same_shape = true;
        for (int i = 0; i < GGML_MAX_DIMS; ++i) {
            same_shape = same_shape && entry.ne[i] == ne[i];
        }
        if (entry.type == type && entry.value_bits == value_bits && same_shape) {
            return entry.tensor;
        }
    }

    ggml_tensor * base = ggml_new_tensor(ctx0, type, GGML_MAX_DIMS, ne);
    ggml_tensor * tensor = ggml_fill(ctx0, base, value);

    constant_cache_entry entry = { type, { ne[0], ne[1], ne[2], ne[3] }, value_bits, tensor };
    constant_cache.push_back(entry);
    return tensor;
}

ggml_tensor * llama_model_deepseek4::graph::get_weight_view_1d(
        ggml_tensor * tensor,
        int64_t ne0,
        int64_t i0) const {
    for (const weight_view_cache_entry & entry : weight_view_cache) {
        if (entry.source == tensor && entry.ne0 == ne0 && entry.i0 == i0) {
            return entry.view;
        }
    }

    ggml_tensor * view = dsv4_view_1d(ctx0, tensor, ne0, i0);
    weight_view_cache.push_back({ tensor, ne0, i0, view });
    return view;
}

static bool dsv4_hoist_zero_rows_enabled() {
    static const bool enabled = [] {
        const char * value = std::getenv("WP_DS4_HOIST_ZERO_ROWS");
        if (value != nullptr && value[0] == '1') {
            LLAMA_LOG_WARN("ds4: WP_DS4_HOIST_ZERO_ROWS=1 can change signed zero and non-finite compression state values\n");
            return true;
        }
        return false;
    }();
...
ggml_tensor * llama_model_deepseek4::graph::build_hc_pre(
    ...
    ggml_tensor * scale_post = get_weight_view_1d(hc_scale, 1, 1);
    ...
    *post = ggml_scale(ctx0, *post, 2.0f);
    ...
    if (cparams.fused_dsv4_hc_comb) {
        *comb = ggml_dsv4_hc_comb(ctx0, mixes, hc_scale, hc_base, hparams.dsv4_hc_eps,
        ...
        ggml_tensor * scale_comb = get_weight_view_1d(hc_scale, 1, 2);
        ggml_tensor * base_comb  = get_weight_view_1d(hc_base, hc*hc, 2*hc);
...
ggml_tensor * llama_model_deepseek4::graph::build_overlap_compressed_kv_from_state(
    ...
    if (dsv4_hoist_zero_rows_enabled()) {
        const int64_t state_row_ne[GGML_MAX_DIMS] = { kv_state->ne[0], 1, 1, 1 };
        kv_state = ggml_concat(ctx0, kv_state, get_constant(kv_state->type, state_row_ne, 0.0f), 1);
        score_state = ggml_concat(ctx0, score_state, get_constant(score_state->type, state_row_ne, -INFINITY), 1);
    } else {
...
// TurboQuant forward-rotate Q. MUST be applied immediately before any DS4
// build_attn_mha call whose K comes from a turbo-typed cache.
//
// WHY THIS EXISTS (2026-08-03). TurboQuant stores K Walsh-Hadamard-rotated --
// k_set_rows_turbo4 applies TURBO_WHT_SIGNS1/2 (ggml/src/ggml-cuda/set-rows.cu
// :1040,:1057) -- and build_attn_mha un-rotates the attention output whenever V
// is a turbo type (llama-graph.cpp:2860,:2945). The FORWARD rotation of Q that
// completes the scheme lives ONLY inside the three build_attn wrappers
// (llama-graph.cpp:3317,:3443,:3638). DS4 calls build_attn_mha DIRECTLY, so it
// never ran: <Q, K_rot> != <Q, K>, every attention score wrong. Measured
// symptom at the config of record: output degenerate and DSpark draft
// acceptance 0.810 -> 0.225. f16 was unaffected because only the turbo store
// path rotates, which is exactly why this looked like "turbo4 is lossy".
//
// ORDERING IS LOAD-BEARING: this must come AFTER DS4's own k_rot Hadamard.
// DS4 applies k_rot symmetrically to q AND kv, so it cancels inside the inner
// product; turbo's WHT is applied to K only (at store time, after k_rot), so Q
// must pick it up last to match.
static ggml_tensor * dsv4_turbo_rotate_q(
        ggml_context * ctx0,
        ggml_tensor * q,
        const ggml_tensor * k,
    ...
    // pad per-head to the 128 WHT group if needed (DS4 is 512-wide, so a no-op)
    if (q->ne[0] % 128 != 0) {
    ...
    return ggml_turbo_wht(ctx0, q, 0, 0, innerq_scale);  // 0 = forward, 0 = auto group
}
...
ggml_tensor * llama_model_deepseek4::graph::build_lid_top_k(
    ...
    const uint32_t n_top_k = ds4_const_shape_enabled()
        ? hparams.indexer_top_k
        : (indexer_score->ne[0] < hparams.indexer_top_k ? indexer_score->ne[0] : hparams.indexer_top_k);
    if (ds4_const_shape_enabled() && indexer_score->ne[0] < n_top_k) {
        const int64_t score_pad_ne[GGML_MAX_DIMS] = {
            n_top_k - indexer_score->ne[0], indexer_score->ne[1], indexer_score->ne[2], indexer_score->ne[3]
        };
        indexer_score = ggml_concat(ctx0, indexer_score,
                get_constant(indexer_score->type, score_pad_ne, -INFINITY), 0);
        cb(indexer_score, "lid_score_padded", il);
    }
    GGML_ASSERT(indexer_score->ne[0] >= n_top_k);
...
ggml_tensor * llama_model_deepseek4::graph::build_top_k_mask(
    ...
    const int64_t kq_mask_all_ne[GGML_MAX_DIMS] = { 1, kq_mask->ne[0], kq_mask->ne[1], kq_mask->ne[3] };
    ggml_tensor * kq_mask_all = ggml_dup(ctx0, get_constant(kq_mask->type, kq_mask_all_ne, -INFINITY));
    ...
    const int64_t zeros_ne[GGML_MAX_DIMS] = { 1, top_k_3d->ne[0], top_k_3d->ne[1], top_k_3d->ne[2] };
    ggml_tensor * zeros = get_constant(cparams.flash_attn ? GGML_TYPE_F16 : GGML_TYPE_F32, zeros_ne, 0.0f);
...
ggml_tensor * llama_model_deepseek4::graph::build_csa_lid_attention(
    ...
    ggml_tensor * csa_kq_mask = inp_csa.kq_mask;
    if (ds4_const_shape_enabled() && csa_kq_mask->ne[0] < top_k->ne[0]) {
        const int64_t mask_pad_ne[GGML_MAX_DIMS] = {
            top_k->ne[0] - csa_kq_mask->ne[0], csa_kq_mask->ne[1], csa_kq_mask->ne[2], csa_kq_mask->ne[3]
        };
        csa_kq_mask = ggml_concat(ctx0, csa_kq_mask,
                get_constant(csa_kq_mask->type, mask_pad_ne, -INFINITY), 0);
    }

    const int64_t n_csa = csa_kq_mask->ne[0];
    GGML_ASSERT(n_csa > 0);
    ...
    ggml_tensor * csa_mask = build_top_k_mask(csa_kq_mask, top_k, "csa_top_k_mask", il);
...
ggml_tensor * llama_model_deepseek4::graph::build_attention_impl(
    ...
    if (ratio == DSV4_CSA_RATIO && inp_dsv4->get_csa().state_pos) {
        ...
        // Restore rollback state, snapshot draft prefixes, and persist token state every step.
        // Only compressed-cache updates wait for a real block boundary.
...
llama_model_deepseek4::graph::graph(const llama_model & model, const llm_graph_params & params) :
    ...
    static const bool const_shape_warning = []() {
        if (ds4_const_shape_enabled()) {
            // 2026-08-24: verify/draft batch WIDTH padding is no longer implied by
            // this flag -- it is WP_SPEC_CONST_WIDTH's decision alone (deprecated).
            // This flag now only pins topology (indexer/CSA top-k KV-length
            // padding here, plus n_stream/index-rank canonicalization in
            // llama-kv-cache-dsv4.cpp); verify masked padding and NLL before
            // combining it with WP_SPEC_CONST_WIDTH.
            LLAMA_LOG_WARN("ds4: WP_DS4_CONST_SHAPE=1 pins decode-graph topology (indexer/CSA top-k KV-length padding, index-vector rank, n_stream); verify masked padding and NLL before use. Combine with WP_SPEC_CONST_WIDTH (deprecated) only for HIP-graph capture experiments\n");
        }
        return true;
    }();
    GGML_UNUSED(const_shape_warning);
    ...
    if (params.gtype == LLM_GRAPH_TYPE_DECODER_MTP && model.fc) {
        GGML_ASSERT(hparams.n_layer_nextn == 3 && "DeepSeek-V4 DSpark expects three stages");
        auto inp = std::make_unique<llm_graph_input_embd>(n_embd);
        inp->tokens = ggml_new_tensor_1d(ctx0, GGML_TYPE_I32, n_tokens);
        ggml_set_input(inp->tokens);
        inp->embd = ggml_new_tensor_2d(ctx0, GGML_TYPE_F32, n_embd, n_tokens);
        ggml_set_input(inp->embd);
        ggml_tensor * input_tokens = inp->tokens;
        ggml_tensor * input_embd   = inp->embd;
        res->add_input(std::move(inp));
        ...
        ggml_tensor * cur = ggml_get_rows(ctx0, model.tok_embd, input_tokens);
        cur = ggml_repeat_4d(ctx0, ggml_reshape_3d(ctx0, cur, n_embd, 1, n_tokens), n_embd, hc, n_tokens, 1);
        for (int il = n_layer; il < hparams.n_layer_all; ++il) {
            const auto & layer = model.layers[il];
            ggml_tensor * residual = cur;
            ggml_tensor * post = nullptr;
            ggml_tensor * comb = nullptr;
            cur = build_hc_pre(cur, layer.hc_attn_fn, layer.hc_attn_scale, layer.hc_attn_base, &post, &comb, il);
            cur = build_norm(cur, layer.attn_norm, nullptr, LLM_NORM_RMS, il);
            cur = build_attention(model, inp_dsv4, cur, inp_pos, il);
            cur = build_hc_post(cur, residual, post, comb, il);

            residual = cur;
            cur = build_hc_pre(cur, layer.hc_ffn_fn, layer.hc_ffn_scale, layer.hc_ffn_base, &post, &comb, il);
        ...
        res->t_logits = cur;
        ggml_build_forward_expand(gf, cur);
        dsv4_build_dspark_head(*this, model, input_tokens);
        return;
    }

    if (params.gtype == LLM_GRAPH_TYPE_DECODER_MTP) {
        GGML_ASSERT(hparams.n_layer_nextn >= 1 && "DeepSeek4 MTP requires at least one block");
