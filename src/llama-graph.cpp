...
ggml_tensor * llm_graph_context::complete_moe_dispatch(ggml_tensor * moe_or_issued,
    ...
    if (moe_dispatch_split_shexp && expert_dispatch != nullptr && il >= 0) {
        ...
        expert_dispatch->mark_wait_tensor(moe_or_issued, il);
...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    if (expert_dispatch != nullptr) {
        ...
        if (moe_out->op == GGML_OP_MAP_CUSTOM3) {
            expert_dispatch->mark_issue_tensor(moe_out, il, 0);
        }
        return moe_out;
    }
...
ggml_tensor * llm_graph_context::build_moe_ffn_ml8(
    ...
    for (uint32_t i = 0; i < hparams.n_expert_used(il); ++i) {
        cur_experts[i] = ggml_view_2d(ctx0, experts, n_embd, n_tokens, experts->nb[2], i*experts->nb[1]);
        ggml_build_forward_expand(gf, cur_experts[i]);
    }

    ggml_tensor * moe_out = cur_experts[0];
    for (uint32_t i = 1; i < hparams.n_expert_used(il); ++i) {
        moe_out = ggml_add(ctx0, moe_out, cur_experts[i]);
        ggml_build_forward_expand(gf, moe_out);
    }
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    if (inp->is_paged) {
        ...
        // 6) wo / wo_b post-projection (mirrors the regular path's tail).
        if (wo) {
