...
const char * llama_ftype_name(llama_ftype ftype) {
    ...
    switch ((enum llama_ftype) (ftype & ~LLAMA_FTYPE_GUESSED)) {
        ...
        case LLAMA_FTYPE_MOSTLY_ML8_FP8:   name = LLAMA_FTYPE_PREFIX "ML8_FP8"; break;
        case LLAMA_FTYPE_MOSTLY_FP8_B128:  name = LLAMA_FTYPE_PREFIX "FP8_B128"; break;
...
llama_model_loader::llama_model_loader(
    ...
    // TODO: make optional
    {
        ...
        switch (type_max) {
            ...
            case GGML_TYPE_ML8_FP8: ftype = LLAMA_FTYPE_MOSTLY_ML8_FP8; break;
            case GGML_TYPE_FP8_B128: ftype = LLAMA_FTYPE_MOSTLY_FP8_B128; break;
            default:
                {
...
struct ggml_tensor * llama_model_loader::create_tensor(
    ...
    if (files.empty()) {
        ...
        if (buft == nullptr) {
            // skipped (TENSOR_SKIP / GGML_OP_NONE), same as the file-backed path
            return nullptr;
        }
    ...
    for (struct ggml_tensor * cur : tensors) {
        ...
        // any duplicates and verify all instances get loaded.
        {
            ...
            if (nm && (std::strstr(nm, "token_embd") != nullptr ||
                       std::strncmp(nm, "output", 6) == 0)) {
                LLAMA_LOG_WARN("[load_all_data] VISIT tensor=%p name=%s data=%p buf=%p size=%zu file_offs=%zu\n",
                               (void*)cur, nm, cur->data, (void*)cur->buffer,
                               ggml_nbytes(cur), (size_t)weight->offs);
            }
        }
