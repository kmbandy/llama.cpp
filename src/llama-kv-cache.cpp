...
llama_kv_cache::llama_kv_cache(
    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            ggml_init_params params = {
                // Reserve tensor metadata for 2 tensors per layer (K and V), plus one view
                // per tensor per stream; +3 for the fork's turbo rotation tensors
                // (turbo_rotation + turbo_rotation_inv + turbo_innerq_scale_inv).
                //
                // 2026-08-10 upstream sync: the per-layer factor drops 3 -> 2 because
                // upstream removed the optional K_idx cache tensor. The fork's +3 stays.
    ...
    if (other) {
        ...
        const bool attn_rot_disable = LLAMA_ATTN_ROT_DISABLE ? atoi(LLAMA_ATTN_ROT_DISABLE) : true;   // TurboQuant: kernel-level WHT; disable upstream graph rotation by default
        if (attn_rot_disable) {
