    ...
    auto ctx_for_buft = [&](ggml_backend_buffer_type_t buft) -> ggml_context * {
        ...
        if (it == ctx_map.end()) {
            ggml_init_params params = {
                // Reserve tensor metadata for up to 3 tensors per layer (K, V, and optional
                // K_idx), plus one view per tensor per stream; +3 for the fork's turbo
                // rotation tensors (turbo_rotation + turbo_rotation_inv + turbo_innerq_scale_inv).
