...
bool llama_model_base::load_tensors(llama_model_loader & ml) {
    ...
    // without mmap, load non-host buffers first: their tensors go through a staging buffer,
    // which is cheapest while the fewest weights are resident (upstream #27483)
    if (!ml.use_mmap) {
                ...
                if (llm_arch_is_recurrent(arch)) {
                    ...
                    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
                        ...
                        res = new llama_memory_hybrid_idx(
                            ...
                            // not an attention kernel, so keep it in F16. Turbo4 cache allocation and
                            // rotated-row handling are not needed for these 128-dim pooled keys.
                    ...
                    if (arch == LLM_ARCH_DEEPSEEK4) {
                        ...
                        res = new llama_kv_cache_dsv4(
                                *this,
                                ...
                                cparams.n_ubatch,
                                1,
                                cparams.n_rs_seq,   // upstream #25784: recurrent-state snapshots for spec rollback
                                filter,
                                reuse,
