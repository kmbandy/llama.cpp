...
static std::vector<llama_kv_cache_dsv4_context::comp_plan> dsv4_build_comp_plans(
    ...
    for (const llama_ubatch & ubatch : ubatches) {
        plans.push_back(llama_dsv4_build_comp_plan(ubatch, ratio, overlap, state_size, kv_size, n_stream, n_rs_seq, rs));
...
static llama_kv_cache_dsv4_context::comp_plan dsv4_build_reserve_comp_plan(
    ...
    // most floor(n_tokens/ratio) + n_seqs  (upstream #26756)
...
llama_kv_cache_dsv4::llama_kv_cache_dsv4(
    ...
    kv_raw = std::make_unique<llama_kv_cache_iswa>(
            ...
            nullptr, filter_raw, reuse, nullptr, filter_authoritative);
...
ggml_tensor * llama_kv_cache_dsv4_raw_context::get_turbo_innerq_scale_inv() const {
    return kv_swa ? kv_swa->get_turbo_innerq_scale_inv() : nullptr;
}
