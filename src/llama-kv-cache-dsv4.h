...
public:
    ...
    // Forward the TurboQuant InnerQ per-channel scale from the underlying cache.
    // The base llama_memory_context_i returns nullptr, which is the identity only
    // while InnerQ is uncalibrated -- relying on that would break silently the
    // moment it is calibrated. Needed by the DS4 graph's forward-WHT of Q.
...
// Live compressor plan. By default, index ranks match the reserve plan so
// compression commits and growing n_visible do not rebuild the graph.
