...
bool common_params_parse(int argc, char ** argv, common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    try {
        ...
        if (!ctx_arg.params.tp_wire_type.empty()) {
            common_set_env("GGML_CUDA_AR_WIRE_TYPE", ctx_arg.params.tp_wire_type);
        }
        params.lr.init();
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    add_opt(common_arg(
        ...
        [](common_params & params) {
            ...
            // use_mmap became the load_mode enum upstream (2026-07-31), so strip
            // only the mmap bit and leave mlock/direct-io intact. This mirrors the
            // order-independent downgrade in common_model_params_to_llama().
            switch (params.load_mode) {
                case LLAMA_LOAD_MODE_MMAP:       params.load_mode = LLAMA_LOAD_MODE_NONE;  break;
                case LLAMA_LOAD_MODE_MMAP_MLOCK: params.load_mode = LLAMA_LOAD_MODE_MLOCK; break;
                default: break;
            }
        }
    ...
    add_opt(common_arg(
        {"--tp-wire-type"}, "{bf16,f16,f32,q8_0,ml8_4,ml8_5,ml8_6}",
        "tensor-parallel AllReduce duplex wire codec (default: bf16)",
        [](common_params & params, const std::string & value) {
            if (value != "bf16" && value != "f16" && value != "f32" && value != "q8_0" &&
                value != "ml8_4" && value != "ml8_5" && value != "ml8_6") {
                throw std::invalid_argument("invalid value");
            }
            params.tp_wire_type = value;
        }
    ).set_env("LLAMA_ARG_TP_WIRE_TYPE").set_examples({LLAMA_EXAMPLE_SERVER, LLAMA_EXAMPLE_CLI, LLAMA_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        {"--spec-draft-n-ctx", "--ctx-size-draft", "-cd"}, "N",
        string_format("context size for the draft/MTP context (0 = inherit from target, capped at target ctx) (default: %d)", params.speculative.draft.n_ctx),
        [](common_params & params, int value) {
    ...
    add_opt(common_arg(
        ...
        string_format(
            "DSpark confidence-head gate: min predicted acceptance of a drafted token; "
            "truncates the block at the first position below it (0.0 = disabled) (default: %.2f)",
            (double) params.speculative.draft.conf_min),
        [](common_params & params, const std::string & value) {
