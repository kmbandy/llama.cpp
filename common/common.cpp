...
common_init_result::common_init_result(common_params & params, bool model_only) :
    ...
    if (params.fit_params) {
        ...
        const bool spec_dspark = std::find(params.speculative.types.begin(), params.speculative.types.end(),
        ...
        if (spec_mtp) {
            ...
            // MAD-LAB: reserve against the self-speculative DSpark graph
        ...
        const common_fit_extra_model extra = {
            ...
            /*.mparams      =*/ &mparams_dft,
            /*.cparams      =*/ &cparams_dft,
            /*.shares_model =*/ !has_draft, // an MTP/DSpark self-spec context runs on the weights of the main model
        };
        ...
        common_fit_params(params.model.path.c_str(), &mparams, &cparams,
            ...
            has_draft || spec_mtp || spec_dspark_self ? &extra : nullptr,
...
struct llama_model_params common_model_params_to_llama(common_params & params) {
    ...
    //
    // NOTE(fork): the 2026-07-31 upstream sync replaced use_mmap/use_mlock/
    // use_direct_io with the single load_mode enum. The old rule was
    // "use_mmap = paging ? false : use_mmap", leaving direct-io and mlock
    // untouched, so strip only the mmap bit here rather than forcing NONE.
    if (params.weight_paging_enabled) {
        switch (mparams.load_mode) {
            case LLAMA_LOAD_MODE_MMAP:       mparams.load_mode = LLAMA_LOAD_MODE_NONE;  break;
            case LLAMA_LOAD_MODE_MMAP_MLOCK: mparams.load_mode = LLAMA_LOAD_MODE_MLOCK; break;
            default: break; // NONE / MLOCK / DIRECT_IO already imply no mmap
        }
    }
    ...
    // MAD-LAB: in-model DSpark uses the same nextn tensor loading path.
    const bool load_mtp = std::find(params.speculative.types.begin(), params.speculative.types.end(), COMMON_SPECULATIVE_TYPE_DRAFT_MTP) != params.speculative.types.end() ||
