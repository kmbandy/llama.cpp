...
struct ggml_tensor * ggml_paged_attn_mt(
    ...
    result->src[0] = q;
    result->src[1] = k_cache;     // mutated in-place by the fused scatter
    result->src[2] = v_cache;     // mutated in-place by the fused scatter
...
// (Legacy ggml_paged_kv_update_mt removed; the fused ggml_paged_attn_mt
// above does the scatter inside its kernel, separated from the attn math
// by an intra-block __syncthreads(). See MAD-114.)
