...
extern bool vk_node_ts_enabled;
extern bool vk_enable_sync_logger;

static inline bool vk_want_timestamps() {
...
void ggml_vk_ctx_begin(vk_device& device, vk_context& subctx, bool one_time = true);
...
// fork: graph plans record into their own descriptor pools/sets
inline std::vector<vk::DescriptorPool> & ggml_vk_descriptor_pools(ggml_backend_vk_context * ctx) {
    return ctx->recording_plan != nullptr ? ctx->recording_plan->descriptor_pools : ctx->descriptor_pools;
}

inline std::vector<vk::DescriptorSet> & ggml_vk_descriptor_sets(ggml_backend_vk_context * ctx) {
    return ctx->recording_plan != nullptr ? ctx->recording_plan->descriptor_sets : ctx->descriptor_sets;
}

inline uint32_t & ggml_vk_descriptor_set_idx(ggml_backend_vk_context * ctx) {
    return ctx->recording_plan != nullptr ? ctx->recording_plan->descriptor_set_idx : ctx->descriptor_set_idx;
}

inline uint32_t & ggml_vk_pipeline_descriptor_set_requirements(ggml_backend_vk_context * ctx) {
    return ctx->recording_plan != nullptr ? ctx->recording_plan->pipeline_descriptor_set_requirements : ctx->pipeline_descriptor_set_requirements;
}

template <typename T>
inline void ggml_vk_dispatch_pipeline(ggml_backend_vk_context* ctx, vk_context& subctx, vk_pipeline& pipeline, std::initializer_list<vk_subbuffer> const& descriptor_buffers, const T &push_constants, std::array<uint32_t, 3> elements) {
    const uint32_t wg0 = CEIL_DIV(elements[0], pipeline->wg_denoms[0]);
    const uint32_t wg1 = CEIL_DIV(elements[1], pipeline->wg_denoms[1]);
    const uint32_t wg2 = CEIL_DIV(elements[2], pipeline->wg_denoms[2]);
    VK_LOG_DEBUG("ggml_vk_dispatch_pipeline(" << pipeline->name << ", {";
    for (auto& buffer : descriptor_buffers) {
        std::cerr << "(" << buffer.buffer->buffer << ", " << buffer.offset << ", " << buffer.size << "), ";
    }
    std::cerr << "}, (" << wg0 << "," << wg1 << "," << wg2 << "))");
    GGML_ASSERT(wg0 <= ctx->device->properties.limits.maxComputeWorkGroupCount[0] &&
                wg1 <= ctx->device->properties.limits.maxComputeWorkGroupCount[1] &&
                wg2 <= ctx->device->properties.limits.maxComputeWorkGroupCount[2]);
    std::vector<vk::DescriptorSet> & descriptor_sets = ggml_vk_descriptor_sets(ctx);
    uint32_t & descriptor_set_idx = ggml_vk_descriptor_set_idx(ctx);
    GGML_ASSERT(descriptor_set_idx < descriptor_sets.size());
    GGML_ASSERT(descriptor_buffers.size() <= MAX_PARAMETER_COUNT);
