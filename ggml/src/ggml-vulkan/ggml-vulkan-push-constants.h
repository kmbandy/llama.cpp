...
struct vk_mat_mat_id_push_constants {
    ...
    // Weight paging. paged=0 keeps stock uniform-stride addressing so the
    // non-paged path is bit-identical. paged=1 means binding 0 is the pager's
    // pool and binding 5 holds each expert's offset within it, in ELEMENTS.
    uint32_t paged;
};
...
template <> inline void init_pushconst_tensor_offsets(ggml_backend_vk_context * ctx, vk_op_turbo_wht_push_constants &p, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * src3, ggml_tensor * dst) {
    p.src_offset   = get_misalign_bytes(ctx, src0) / ggml_type_size(src0->type);
    p.dst_offset   = get_misalign_bytes(ctx, dst)  / ggml_type_size(dst->type);
...
template <> inline void init_pushconst_tensor_offsets(ggml_backend_vk_context * ctx, vk_op_sinkhorn_norm_push_constants &p, const ggml_tensor * src0, const ggml_tensor * src1, const ggml_tensor * src2, const ggml_tensor * src3, ggml_tensor * dst) {
