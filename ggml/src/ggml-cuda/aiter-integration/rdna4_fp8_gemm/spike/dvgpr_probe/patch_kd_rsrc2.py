#!/usr/bin/env python3
"""
patch_kd_rsrc2.py - set COMPUTE_PGM_RSRC2 bit 6 (ENABLE_DYNAMIC_VGPR) in a HIP
code object's kernel descriptor, producing a patched copy. NO GPU, NO LAUNCH.

WHY THIS, NOT patch_kd.py:
  patch_kd.py set COMPUTE_PGM_RSRC3 bit 17 - that is the *gfx1250* (GFX125)
  dynamic-VGPR enable, which is RESERVED on gfx1201. The earlier silicon probe
  correctly saw it ignored (DYN_VGPR_EN=0). The *RDNA4* (gfx1200/gfx1201 = GFX120)
  enable is a DIFFERENT bit in a DIFFERENT register:

    COMPUTE_PGM_RSRC2.ENABLE_DYNAMIC_VGPR = bit 6 (width 1)   [GFX120 only]

  This is the bit that is ENABLE_TRAP_HANDLER on GFX6-GFX11. Verified against
  /opt/rocm/llvm/include/llvm/Support/AMDHSAKernelDescriptor.h (ROCm 7.2.3):
    157:  COMPUTE_PGM_RSRC2_GFX6_GFX11(ENABLE_TRAP_HANDLER, 6, 1),
    158:  COMPUTE_PGM_RSRC2_GFX120(ENABLE_DYNAMIC_VGPR,    6, 1),
  and documented for GFX120* in the ROCm 7.13.0-preview LLVM AMDGPUUsage doc:
    "Enables dynamic VGPR mode ... Used by CP to set up
     COMPUTE_PGM_RSRC2.DYNAMIC_VGPR."

Kernel descriptor layout (AMDHSA, static_assert-verified in the header):
  COMPUTE_PGM_RSRC2 is at byte offset 52 (COMPUTE_PGM_RSRC2_OFFSET = 52).
...
section, then flip bit 6 of the uint32 at (kd_file_offset + 52).
"""
...
KD_SIZE = 64
COMPUTE_PGM_RSRC2_OFFSET = 52   # static_assert-verified in AMDHSAKernelDescriptor.h
ENABLE_DYNAMIC_VGPR_BIT = 6     # GFX120 (RDNA4) enable; ENABLE_TRAP_HANDLER on GFX6-11
...
def find_kd_symbol(elf):
    """Parse a little-endian ELF64; return (name, st_value, st_shndx, kd_file_off)."""
...
def main():
    ...
    dst = sys.argv[2] if len(sys.argv) > 2 else "probe_rsrc2_patched.hsaco"
    ...
    rsrc2_abs = kd_abs + COMPUTE_PGM_RSRC2_OFFSET

    before, = struct.unpack_from("<I", data, rsrc2_abs)
    ...
    print(f"compute_pgm_rsrc2 @  : 0x{rsrc2_abs:x}  (= kd 0x{kd_abs:x} + {COMPUTE_PGM_RSRC2_OFFSET})")
    print(f"compute_pgm_rsrc2 BEFORE : 0x{before:08x}")
    print(f"compute_pgm_rsrc2 AFTER  : 0x{after:08x}   (bit {ENABLE_DYNAMIC_VGPR_BIT} set)")

    if before == after:
        print("WARNING: bit 6 already set in source; output will be identical.")

    struct.pack_into("<I", data, rsrc2_abs, after)

    with open(dst, "wb") as f:
        f.write(data)

    # sanity: diff source vs patched - must differ in exactly one bit
    with open(src, "rb") as f:
        orig = f.read()
    ...
    expected_byte = rsrc2_abs + (ENABLE_DYNAMIC_VGPR_BIT // 8)
