// Long-context AITER 3D+reduce test:
//   - ctx=1024 (64 paged blocks @ block_size=16)
//   - K is 2-class: tokens [0..ctx/2) have K=0.0, [ctx/2..ctx) have K=1.0
//     → QK scores: class A = 0,  class B = scale*head_size = 0.0883883*128 = 11.31
//     → softmax weight for class A = exp(0)/(exp(0)+exp(11.31)) ≈ 1.2e-5 (negligible in fp16)
//     → softmax weight for class B ≈ 1/N_B where N_B = ctx/2 = 512
//   - V[blk, tok, kvh, d] = (kvh+1) * (d/128) * (gtok/ctx)  (same as multi-block test)
//
// Expected output:
//   out[h, d] ≈ mean_{t in B} V[t, kvh(h), d]
//             = (kvh+1) * (d/128) * mean_{t=ctx/2..ctx-1}(t/ctx)
//
//   mean_{t=512..1023}(t/1024) = (sum t=512..1023) / (512*1024)
//                              = ((1023*1024/2) - (511*512/2)) / (512*1024)
//                              = (523776 - 130816) / 524288
//                              = 392960 / 524288
//                              = 0.7494812...
//
//   So expected[h, d] = (kvh+1) * (d/128) * 0.7494812
//
// This exercises:
//   ✓ 64-block paged walk
//   ✓ Non-uniform softmax with one class effectively masked out
//   ✓ Long-context behavior (real decode-scale ctx)
//   ✓ Split-K across all 32 segments
//   ✓ Numerical stability of softmax (large positive scores)
...
int main() {
    ...
    constexpr int NUM_BLOCKS         = CTX_LEN / BLOCK_SIZE;     // 64
    ...
    // K: 2-class step. First half of tokens K=0, second half K=1.
    ...
    // V varies by global token position (same as multi-block test)
    ...
    printf("Long-ctx test: ctx=%d (%d blocks, %d active segments), 2-class K (half=0/half=1)\n",
           CTX_LEN, NUM_BLOCKS, NUM_SEGMENTS_PER_SEQ);
    printf("  Q=1.0, K2nd-half=1.0, QK score for class B = scale*head_size = %.4f\n",
           SCALE * HEAD_SIZE);
    printf("  Expected softmax weight on class A ≈ exp(0)/(exp(0)+exp(%.2f)) ≈ %.2e (negligible)\n",
           SCALE * HEAD_SIZE, std::exp(0.0) / (std::exp(0.0) + std::exp(SCALE * HEAD_SIZE)));
    ...
    // Compute reference: full softmax on class B only (class A weight is negligible)
    // mean_{t=ctx/2..ctx-1}(t/ctx) = ((ctx-1) + ctx/2) / (2*ctx)
    // for ctx=1024: (1023+512)/(2*1024) = 1535/2048 = 0.7495117  (close to 0.7494812)
    // Be more precise: arithmetic sum
    double ref_mean = 0.0;
    int n_class_b = 0;
    ...
    printf("  hand-computed mean_{class B}(tok/ctx) = %.7f (N=%d tokens)\n", ref_mean, n_class_b);
    ...
    for (int h = 0; h < NUM_Q_HEADS; ++h) {
        ...
        for (int d = 0; d < HEAD_SIZE; ++d) {
            ...
            float expected = (kvh + 1.0f) * ((float)d / (float)HEAD_SIZE) * (float)ref_mean;
    ...
    // Sample print a few values
    printf("\nsample outputs (h, d) → actual / expected:\n");
    int samples[][2] = {{0, 0}, {0, 64}, {0, 127}, {8, 0}, {8, 64}, {15, 127}};
    for (auto& s : samples) {
        int h = s[0], d = s[1], kvh = h / (NUM_Q_HEADS / NUM_KV_HEADS);
        float actual = fp16_to_float(out_h[(size_t)h * HEAD_SIZE + d]);
        float expected = (kvh + 1.0f) * ((float)d / (float)HEAD_SIZE) * (float)ref_mean;
        printf("  h=%2d d=%3d (kvh=%d):  actual=%.5f  expected=%.5f\n", h, d, kvh, actual, expected);
    }

    bool pass = (nan_count == 0) && (sentinel_count == 0) && (mismatch_count == 0);
    printf("\n%s\n", pass ? "✓ AITER long-context 3D+reduce test PASSED" : "✗ AITER long-context 3D+reduce test FAILED");
