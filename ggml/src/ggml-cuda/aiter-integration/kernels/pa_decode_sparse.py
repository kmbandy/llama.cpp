...
#
#   Source:   aiter/ops/triton/_triton_kernels/attention/pa_decode_sparse.py
#   Upstream: https://github.com/ROCm/aiter
#   Repo HEAD at vendoring time: c1d1476cf36f13b00b430e4fcd331f1979f30461 (2026-09-23)
#   Last commit touching this file: dc100157521a2557629a36e2bcfb2d178f571dce (2026-08-11)
#
...
# decode half (split-K main kernel + reduce). Both `_pa_decode_sparse` and
# `_pa_decode_sparse_reduce` are carried over verbatim — the reduce kernel
# IS needed here (unlike the prefill file's dropped index-packing kernels):
# Stage 1 microbenchmark confirmed DS4.1's real decode shape
# (H=64, D=512, N_IDX=640, BLOCK_K=16) resolves KV_SPLITS=64 > 1, so the
# split-K path is live and its partials must be combined.
#
# ─────────────────────────────────────────────────────────────────────────
# LOCAL PATCHES APPLIED (deviations from upstream) — import/adapter only,
# no autotune decorator was present upstream to strip:
#
# 1. Dropped `make_kernel_repr` import
#    (`aiter.ops.triton.utils._triton.kernel_repr`) and the
#    `_pa_decode_sparse_repr` / `_pa_decode_sparse_reduce_repr` module-level
#    calls that used it — cosmetic only (kernel-launch logging labels), and
#    the import pulls in the aiter package for a single string-formatting
#    helper we don't need at AOT-compile time (same rationale as the
#    upstream unified_attention.py vendor's e4m3_dtype patch in this same
#    directory). `@triton.jit` (bare) replaces `@triton.jit(repr=...)` on
#    both kernels.
#
# No other line of either kernel's body was touched.
# ─────────────────────────────────────────────────────────────────────────
