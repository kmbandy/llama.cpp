...
struct server_task_result_metrics : server_task_result {
    ...
    // MAD-LAB: series that depend on live subsystem state (paged KV tier, weight
    // pager). Filled by the METRICS task handler, which is the only place with a
    // context/model to read them off; appended verbatim by to_metrics().
    std::vector<metric_item> extra_counters;
    std::vector<metric_item> extra_gauges;
